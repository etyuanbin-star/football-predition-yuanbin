...
with st.expander("🤔 互动问答：测试你的理解"):
    ...
    if st.button("提交答案"):
        ...
        if q2 == "B. 负期望值，会长期亏损":
            correct += 1
            st.success("✅ 正确！55% × 1.90 = 1.045，期望值4.5%，但考虑投注额...等等，再算算")
            st.info("正确计算：期望值 = (0.55 × 1.90 - 1) × 100% = 4.5%，这是正期望值。但注意：这个胜率很难长期维持。")
        else:
            st.error("❌ 错误。0.55 × 1.90 - 1 = 0.045，是正期望值，但55%胜率很难达到。")
